...
def lambda_handler(event, context):
    with conn.cursor() as cur:
        ...
        for node in node_response:
            ...
            todo = {"jsonrpc": "2.0", "method": "platform.getTotalStake","id": 1 }
            ...
            mainResult = {'jsonrpc': jData.get("jsonrpc"), 'result':{"stake": result.get("stake"),"nodeId": node[4]}, 'id': jData.get("id")}
            messageData = {"messageType":"TotalStake","blockchainType":"Avalanche","timestamp":time.time(),"body":mainResult}
            print(messageData)
